...
- `Динамика среднего score` is now built for the latest 3 months, while the compact recognition/tokens/minutes charts remain limited to the latest 7 days.
...
- Dashboard additionally shows 3-month heatmaps for all calls and recognized calls, 3-month dynamics for high-risk calls and tokens-per-minute, and a bar chart for the most frequently violated scenario checkpoints.
- Dashboard now uses separate charts for call sentiment and risk distribution, a dedicated line chart for calls without recordings, and a simplified horizontal month label row above each 3-month heatmap.
