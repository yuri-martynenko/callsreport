...
- Dashboard includes a scenario-level `tokens per minute` bar chart for the whole period and keeps it together with the monthly overall `tokens per minute` line chart and the monthly token-usage line chart in one dedicated row immediately after the 6-month heatmap. The heatmap groups are now rendered per month, so days at month edges no longer spill visually into the next month. The `Распознанные звонки` and `Распознанные минуты` monthly charts are rendered as a 50/50 row across the full page width, with lighter point-value labels.
