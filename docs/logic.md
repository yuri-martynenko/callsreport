...
- Dashboard now shows a combined 3-month line chart for `Количество вызовов`, `Распознанные вызовы`, and `Без записи`, while the compact recognition/tokens/minutes charts and `Токены на 1 минуту звонка` are limited to the latest 1 month.
...
- Dashboard additionally shows a single 6-month heatmap for all calls, a 1-month dynamics chart for tokens-per-minute, and a bar chart for the most frequently violated scenario checkpoints.
- The 6-month heatmap now renders per-day call counts directly inside filled cells; zero-value days remain empty.
- Dashboard now uses separate charts for call sentiment and risk distribution, folds `calls without recordings` into the combined 3-month call-volume chart, keeps all three lines solid, uses the original grouped heatmap layout with weekly columns split by month, positions month captions at the exact start column of each month, and renders monthly line-chart dates in a shortened sparse format to avoid overlaps.
- `GET /api/dashboard-charts` now builds its payload from the cached raw activity snapshot plus the analysis store instead of the heavier full filtered-calls pipeline, which reduces startup cost for the dashboard view.
- Dashboard includes a scenario-level `tokens per minute` bar chart for the whole period, where each scenario is calculated as `sum(totalTokens) / sum(transcriptMinutes)` across its calls, but any call shorter than `60` seconds contributes `1` full minute to the denominator to avoid extreme minute-cost spikes on ultra-short calls. It stays together with the monthly overall `tokens per minute` line chart and the monthly token-usage line chart in one dedicated row immediately after the 6-month heatmap. The heatmap groups are now rendered per month, so days at month edges no longer spill visually into the next month, and the heatmap card spans the full dashboard width. The `Распознанные звонки` and `Распознанные минуты` monthly charts are rendered as a 50/50 row across the full page width, with lighter point-value labels.
- The dashboard bar charts `Тональность звонков`, `Средний процент по сценариям`, `Средний балл по менеджерам`, and `Топ нарушений` now follow one display rule: no secondary hint line, non-bold labels, a 0–100 scale, and right-side values shown as `count + percent`.
...
- `/api/dashboard-charts` now returns pre-aggregated chart series from backend, so the browser no longer waits for a large `dashboardCalls` payload and repeated client-side aggregation passes over the full call list.
