...
- Dashboard now shows a combined 3-month line chart for `Количество вызовов` and `Распознанные вызовы`, while the compact recognition/tokens/minutes charts remain limited to the latest 7 days.
...
- Dashboard additionally shows a single 3-month heatmap for all calls, 3-month dynamics for tokens-per-minute and calls without recordings, and a bar chart for the most frequently violated scenario checkpoints.
- Dashboard now uses separate charts for call sentiment and risk distribution, a dedicated line chart for calls without recordings, and the original grouped heatmap layout with weekly columns split by month.
