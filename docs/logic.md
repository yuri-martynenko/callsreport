...
- `GET /api/calls` uses a short-lived in-memory cache for the filtered and sorted call snapshot, so adjacent page requests with the same filters reuse the prepared dataset instead of refetching and resorting everything.
