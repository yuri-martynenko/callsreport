...
- Dashboard also includes a scenario-level `tokens per minute` bar chart for the whole period, a separate scenario-level `tokens per minute` bar chart for the last month, and keeps these together with the monthly overall `tokens per minute` line chart in one dedicated row immediately after the 6-month heatmap. The `Распознанные звонки` and `Распознанные минуты` monthly charts are rendered as a 50/50 row across the full page width.
