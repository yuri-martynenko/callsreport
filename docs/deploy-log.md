...
- Длительность: около 15 минут.
- Результат: `push` коммита `30b3807` выполнен в `main`, production deploy завершился успешно со статусами `stop_existing/clean/download/normalize_windows_paths/cleanup_metadata/runtime/install/systemd/start/healthcheck = ok`, внешний `GET https://app-2f37df5d.vibecode.bitrix24.tech/api/health` через access token вернул `ok=true, configured=true`, локальный server-side `GET http://127.0.0.1:3000/api/health` вернул `ok=true, configured=true`, production-файлы подтвердили наличие `accessRolesTable`, `accessRoleModal`, `open-access-role` и `access-permission-mark`.

## 2026-04-25 17:39 VLAT

- Задача: deploy унификации расчёта месячного графика `Токены на 1 минуту звонка за последний месяц`: для звонков короче `60` секунд считать знаменатель как `1` минуту, по той же логике, что уже используется в графике `Токены на 1 минуту по сценариям`.
- Длительность: около 10 минут.
- Результат: `push` runtime-коммита `53fe59c` выполнен в `main`, production deploy завершился успешно со статусами `stop_existing/clean/download/normalize_windows_paths/cleanup_metadata/runtime/install/systemd/start/healthcheck = ok`, внешний `GET https://app-2f37df5d.vibecode.bitrix24.tech/api/health` вернул `ok=true, configured=true`, а `GET https://app-2f37df5d.vibecode.bitrix24.tech/api/dashboard-charts` подтвердил обновлённую точку `tokensPerMinuteSeries` для `2026-04-25 = 2010` вместо прежних `4307.1`.

## 2026-04-25 17:08 VLAT

- Задача: deploy настройки доступов к приложению: роли сотрудников, список пользователей с портала, права на страницы `Дашборд` / `Расшифровка` / `Сценарии`, ручной запуск расшифровки, режим авторасшифровки и редактирование сценариев.
...
- Длительность: около 15 минут.
- Результат: `push` runtime-коммитов `73bd85e` и `da2aa5b` выполнен в `main`, production deploy финального runtime-коммита `da2aa5b` завершился успешно со статусами `stop_existing/clean/download/normalize_windows_paths/cleanup_metadata/runtime/install/systemd/start/healthcheck = ok`, внешний `GET https://app-2f37df5d.vibecode.bitrix24.tech/api/health` вернул `ok=true, configured=true`, production HTML подтвердил обновлённый subtitle `Средний общий расход токенов транскрибации и анализа...`, а `GET /api/dashboard-charts` подтвердил актуальную формулу для `scenarioTokensPerMinuteRows`.

## 2026-04-25 16:28 VLAT

- Задача: deploy коррекции dashboard после повторной обратной связи: вернуть третий токеновый график к виду `Израсходование токенов за последний месяц`, перевести `Токены на 1 минуту по сценариям` на среднее значение `tokens/min` по звонкам, переразбить heatmap `Тепловая карта вызовов за 6 месяцев` по отдельным месяцам без визуального перетекания дней и убрать жирность подписей значений у графиков `Распознанные звонки за последний месяц` и `Распознанные минуты за последний месяц`.
- Длительность: около 20 минут.
- Результат: `push` коммита `d313480` выполнен в `main`, production deploy завершился успешно со статусами `stop_existing/clean/download/normalize_windows_paths/cleanup_metadata/runtime/install/systemd/start/healthcheck = ok`, внешний `GET https://app-2f37df5d.vibecode.bitrix24.tech/api/health` вернул `ok=true, configured=true`, production `app.js` подтвердил маркеры `groupedByMonth`, `is-emphasis-light`, `tokensUsageSeries`, `rateSum`, `rateCount`, а `GET /api/dashboard-charts` подтвердил `30` точек в `tokensUsageSeries`, `recognizedCallsSeries`, `recognizedMinutesSeries`.
...
- Длительность: около 20 минут.
- Результат: `push` коммита `10bd2c4` выполнен в `main`, production deploy завершился успешно со статусами `stop_existing/clean/download/normalize_windows_paths/cleanup_metadata/runtime/install/systemd/start/healthcheck = ok`, внешний `GET https://app-2f37df5d.vibecode.bitrix24.tech/api/health` вернул `ok=true, configured=true`, production `app.js` подтвердил `positionedMonthGroups`, `left:${group.offset}px` и `requestIdleCallback`, а контрольный `GET /api/dashboard-charts` после deploy выполнился примерно за `1680 ms`.

## 2026-04-25 09:52 VLAT

- Задача: deploy дополнительной коррекции dashboard после визуальной проверки: привязать подписи месяцев в heatmap `Тепловая карта вызовов за 6 месяцев` к неделе, содержащей первое число месяца, и проредить/укоротить подписи дат у месячных графиков (`Токены на 1 минуту звонка`, `Распознанные звонки`, `Израсходование токенов`, `Распознанные минуты`) для читаемой временной оси.
- Длительность: около 15 минут.
- Результат: `push` коммита `352fd28` выполнен в `main`, production deploy завершился успешно со статусами `stop_existing/clean/download/normalize_windows_paths/cleanup_metadata/runtime/install/systemd/start/healthcheck = ok`, внешний `GET https://app-2f37df5d.vibecode.bitrix24.tech/api/health` вернул `ok=true, configured=true`, а production `app.js` подтвердил новую логику `monthStarts` для heatmap и `formatDayShort` + `maxXAxisLabels: 6` для месячных графиков.

## 2026-04-25 09:45 VLAT

- Задача: deploy коррекции dashboard после повторной проверки: выровнять подписи месяцев над heatmap `Тепловая карта вызовов за 6 месяцев` по первой неделе месяца и перевести backend-агрегации графиков `Токены на 1 минуту звонка`, `Распознанные звонки`, `Израсходование токенов`, `Распознанные минуты` на период `1 месяц`.
- Длительность: около 15 минут.
- Результат: `push` коммита `e8f2544` выполнен в `main`, production deploy завершился успешно со статусами `stop_existing/clean/download/normalize_windows_paths/cleanup_metadata/runtime/install/systemd/start/healthcheck = ok`, внешний `GET https://app-2f37df5d.vibecode.bitrix24.tech/api/health` вернул `ok=true, configured=true`, `GET /api/dashboard-charts` подтвердил длину нужных серий в `30` точек, а production-файлы подтвердили `gap: 12px` и `width: fit-content` у `.heatmap-months` и заголовки графиков с периодом `за последний месяц`.

## 2026-04-25 08:35 VLAT

- Задача: deploy правок dashboard с выравниванием подписей месяцев над heatmap `Тепловая карта вызовов за 6 месяцев`, переводом названий месяцев в полный формат и сменой периода четырёх графиков (`Токены на 1 минуту звонка`, `Распознанные звонки`, `Израсходование токенов`, `Распознанные минуты`) с `7 дней`/`3 месяца` на `1 месяц`.
- Длительность: около 15 минут.
- Результат: `push` коммита `92c9ed8` выполнен в `main`, production deploy завершился успешно со статусами `stop_existing/clean/download/normalize_windows_paths/cleanup_metadata/runtime/install/systemd/start/healthcheck = ok`, внешний `GET https://app-2f37df5d.vibecode.bitrix24.tech/api/health` и локальный `GET http://127.0.0.1:3000/api/health` на сервере вернули `ok=true, configured=true`, а production-файлы подтвердили `month: "long"`, width-binding через `--columns` для `.heatmap-month-label` и расчёт нужных серий по `30` дням.
