...
Backend при старте ищет `.env` в нескольких местах:

- в директории релиза;
- в родительской директории релиза;
- в `process.cwd()`;
- в `/opt/app/.env`.

Для production канонический файл по-прежнему должен лежать в `/opt/app/.env`, чтобы Vibe Deploy не зависел от того, как именно создан systemd unit и из какого каталога запущен `node server.js`.
...
5. Убедиться, что `VIBE_API_KEY` и остальные обязательные переменные доступны приложению через `/opt/app/.env` или уже экспортированы в окружение процесса.
6. Перезапустить процесс `node server.js`.
