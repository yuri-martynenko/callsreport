...
Endpoint используется для мультиселектов в модальном окне сценария. Backend собирает опции из сохраненных сценариев и доступных звонков, чтобы пользователь выбирал значения из списка, а не вводил служебные идентификаторы вручную.

Для `pipelines` и `stages` backend дополнительно обогащает звонки данными CRM-сущностей:

- для `deal` используется `categoryId` и `stageId` сделки;
- для `lead` используется `stageId` лида.
